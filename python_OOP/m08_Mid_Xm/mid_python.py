...
#         else:
#             print("Hall can be added.")

# class Hall(Star_Cinema):
#     def __init__(self, rows, cols, hall_no, hall_name):
#         self.__seats = {}  
#         self.__show_list = []  
#         self.__rows = rows  
#         self.__cols = cols  
...
#                     print(f"Seat ({row}, {col}) booked for show {show_id}, hall no {self.__hall_no}, hall name: {self.__hall_name}")
#                 else:
...
#     def book_tickets(self, hall_id, show_id, seat_list):
#         found = False
#         for hall in self.__halls:
#             if hall._Hall__hall_no == hall_id and show_id in hall._Hall__seats:
#                 hall._book_seats(show_id, seat_list)
#                 found = True
#                 break
#         if not found:
#             print(f"Error: Show ID {show_id} not found for booking in Hall {hall_id}.")
...
#             counter.book_tickets(hall_id, show_id, seats)
...
# Anupom = Hall(6, 5, '01', 'Anupom Hall')  
...
class Star_Cinema:
    ...
    def _entry_hall(self, hall):
        if isinstance(hall, Hall):
            ...
            print("Invalid hall, cannot be added.")
...
def main_menu():
    ...
    while True:
        ...
        if option == '1':
            ...
            hall_id = input("Enter Hall ID: ").strip()
            show_id = input("Enter Show ID: ").strip()
            if hall_id and show_id:
                num_tickets = int(input("Number of Tickets: "))           
                seats = []
                for _ in range(num_tickets):
                    row = int(input("Enter Seat Row: "))
                ...
                print("Hall ID and Show ID cannot be empty.")
...
# Sample Halls and Shows
Anupom = Hall(6, 5, 1, 'Anupom Hall')  
Anupom._entry_show('111', 'Jawan Maji', '25/09/2024 9:00 AM')
Anupom._entry_show('333', 'Sujon Maji', '25/09/2024 2:00 PM')
Anupom._entry_show('555', 'Monpura', '25/09/2024 6:00 PM')

Sapla = Hall(12, 8, 2, 'Sapla Hall')  
Sapla._entry_show('111', 'Jawan Maji', '25/10/2023 11:00 AM')
Sapla._entry_show('333', 'Sujon Maji', '25/10/2023 2:00 PM')

Modhubon = Hall(10, 16, 4, 'Modhubon Hall')  
Modhubon._entry_show('111', 'Jawan Maji', '25/10/2023 11:00 AM')
Modhubon._entry_show('333', 'Sujon Maji', '25/10/2023 2:00 PM')
